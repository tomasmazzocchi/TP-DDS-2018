...
public class IndicadorController {
	...
	public static Void guardarIndicador(Request request, Response response) {
			...
			Empresa empresaSeleccionada = empresas.stream().filter(x -> x.getNombre().equals(request.queryParams("selected"))).findFirst().get();
			IndicadorDTO indicador = new IndicadorDTO(request.queryParams("nombre"),request.queryParams("formula"),empresaSeleccionada.getId(),usuario);
			if(indicador.getNombre().isEmpty() || indicador.getFormula().isEmpty() || indicador.getNombre() == null || indicador.getFormula() == null){
	...
	public static ModelAndView calcularIndicador(Request request, Response response) {
		...
		Empresa empresaSeleccionada = empresas.stream().filter(x -> x.getNombre().equals(request.queryParams("selectedEmp"))).findFirst().get();
		IndicadorDTO indicadorSeleccionado = listaIndicadores.stream().filter(x -> x.getNombre().equals(request.queryParams("selectedInd"))).findFirst().get();
